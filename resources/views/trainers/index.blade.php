...
<div class="flex flex-wrap justify-center mt-10">
    ...
    <div class="p-4 max-w-sm">
        <div class="flex rounded-lg h-full dark:bg-gray-800 bg-teal-400 p-8 flex-col">
            <img class="w-24 h-24 mb-3 rounded-full shadow-lg" src="{{ asset('storage/' . $trainer->image)}}" alt="{{ $trainer->name }}"/>
            <h5 class="mb-1 text-xl font-medium text-gray-900 dark:text-white">{{ $trainer->name }}</h5>
            <span class="text-sm text-gray-500 dark:text-gray-400">{{ $trainer->rank }}</span>
